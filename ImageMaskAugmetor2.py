...
class ImageMaskAugmentor:
  ...
  def __init__(self, config_file):
     ...
     self.radii      = self.config.get("augmentor",  "radii", dvalue=[0.3])
  ...
  def augment(self):
     ...
     for i in range(num_image_files):
        ...
        self.distort(image_file, mask_file) #, amount, radius)


  def distort(self, image_file, mask_file): #, amount, radius):
    ...
    for amount in self.amounts:
      for radius in self.radii:
        self.distort_one(image, mask, image_basename, mask_basename, amount, radius)


  def distort_one(self, image, mask, image_basename, mask_basename, amount, radius):
    (h,  w,  _) = image.shape
    (hm, wm, _) = mask.shape
    if h != hm or w != wm:
